...
/**
 * Uniform content padding for a screen's outermost scrollable container
 * (`LazyColumn`/`LazyVerticalGrid`). The system navigation bar's bottom
 * inset is *not* handled here — it's reserved once for every screen at the
 * app root (`AppNavigation`), so this only carries the visual screen margin.
 * Screens should still use this instead of a bare `PaddingValues(...)` so
 * the margin stays consistent app-wide.
 */
fun ursScreenContentPadding(
    horizontal: Dp = Spacing.l,
    vertical: Dp = Spacing.l,
): PaddingValues = PaddingValues(start = horizontal, end = horizontal, top = vertical, bottom = vertical)
