...
fun ProductListScreen(
    ...
    viewModel: ProductsViewModel = viewModel(
        factory = ProductsViewModel.factory(inventoryId, inventoryName, stringResource(R.string.inventory_uncategorized)),
    ),
) {
    ...
    Box(modifier = Modifier.fillMaxSize()) {
        Column(Modifier.fillMaxSize()) {
            ...
            when (val state = uiState) {
                ...
                is ProductsUiState.Data -> ProductGrid(
                    groups = state.groups,
...
private fun ProductGrid(
    groups: List<InventoryCategoryGroup>,
    onOpenSettings: (InventoryProductTile) -> Unit,
) {
    if (groups.isEmpty()) {
    ...
    LazyVerticalGrid(
        ...
        groups.forEach { group ->
            item(key = "header-${group.categoryName}", span = { GridItemSpan(maxLineSpan) }) {
                UrsText(
                    group.categoryName,
            ...
            gridItems(group.products, key = { it.product.id }) { tile ->
                ProductTile(tile = tile, onClick = { onOpenSettings(tile) })
            }
        }
    }
}
...
@Composable
private fun ProductTile(tile: InventoryProductTile, onClick: () -> Unit) {
    val product = tile.product
    val quantity = product.quantity
    // Below either threshold (whichever is set) draws the same hazard
    // border as the shopping list's "only buy on sale" marker — the tile
    // format has no room for the row layout's separate orange/red warning
    // colors, so both threshold levels collapse into one visual cue here.
    val belowThreshold = quantity != null &&
        ((product.secondThreshold != null && quantity <= product.secondThreshold) ||
            (product.firstThreshold != null && quantity <= product.firstThreshold))
    val syncBadge = when (product.syncStatus) {
        SyncStatus.PENDING -> stringResource(R.string.fill_status_pending)
        SyncStatus.FAILED -> stringResource(R.string.fill_status_failed)
    ...
    UrsSquareTile(
        ...
        onClick = onClick,
        quantityBadge = quantity?.toString() ?: stringResource(R.string.inventory_product_not_tracked),
        topEndBadge = syncBadge,
        hazardBorder = belowThreshold,
    )
}
...
private fun ProductSettingsForm(form: ProductSettingsFormState, viewModel: ProductsViewModel) {
    ...
    Column(
        ...
        // Quick-adjust buttons — ±1 for a single unit, ±7 for weekly-batch
        // products (e.g. medication prepared once a week). Fixed steps, not
        // configurable.
        Row(horizontalArrangement = Arrangement.spacedBy(Spacing.m)) {
            UrsOutlinedButton(
                text = stringResource(R.string.inventory_settings_quantity_minus_seven),
                onClick = { viewModel.adjustSettingsQuantity(-7) },
                modifier = Modifier.weight(1f),
            )
            UrsOutlinedButton(
                text = stringResource(R.string.inventory_settings_quantity_minus_one),
                onClick = { viewModel.adjustSettingsQuantity(-1) },
                modifier = Modifier.weight(1f),
            )
            UrsOutlinedButton(
                text = stringResource(R.string.inventory_settings_quantity_plus_one),
                onClick = { viewModel.adjustSettingsQuantity(1) },
                modifier = Modifier.weight(1f),
            )
            UrsOutlinedButton(
        ...
        // The tile grid has no inline delete button any more (mcfx-urs/urs-android#100) — this is now the only way to remove a tracked product.
        UrsOutlinedButton(
            text = stringResource(R.string.inventory_delete),
            onClick = viewModel::deleteSettingsProduct,
            modifier = Modifier.fillMaxWidth(),
        )
    }
}
